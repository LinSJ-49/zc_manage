...
//学科列表数据
const subjects = ref([{ name: 'Java', value: 1 },{ name: '前端', value: 2 },{ name: '大数据', value: 3 },{ name: 'Python', value: 4 },{ name: '后端', value: 5 },{ name: '嵌入式', value: 6 }])
//搜索表单对象
...
onMounted(() => {
  queryPage()
  queryAllEmp()
   console.log('emps.value:', emps.value); 
})
...
const queryAllEmp = async () => {
  ...
  console.log('result:', result);  // 打印 result
  if(result.code) {
    emps.value = result.data
    console.log('emps.value:', emps.value);  // 打印 emps.value
  }
}
...
<template>
    ...
    <el-table :data="tableData" border style="width: 100%" fit>
      ...
      <el-table-column prop="masterName" label="授课教师" align="center" width="100px"/>
  ...
  <el-dialog v-model="dialogFormVisible" :title="formTitle" width="35%">
    <el-form :model="clazz" ref="clazzFormRef" :rules="rules">
      ...
      <el-form-item label="授课教师" :label-width="labelWidth">
        <el-select v-model="clazz.masterId" placeholder="请选择班授课教师" style="width: 100%;">
