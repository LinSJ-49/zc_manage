...
//学历列表数据
const degrees = ref([{ name: '大专', value: 1 },{ name: '本科', value: 2 }])
//性别列表数据
...
let tableData = ref([])
// 按学号排序
const sortByStudentNumber = () => {
  tableData.value.sort((a, b) => {
    return a.no.localeCompare(b.no);
  });
}
const setDefaultSort = () => {
  queryPage(); // 调用 queryPage 方法恢复初始状态
}
...
const rules = ref({
  ...
  isCollege: [
    { required: true, message: '是否院校学生为必填项', trigger: 'change' }
  ]
})
...
<template>
    ...
    <!-- 功能按钮 -->
    <el-button type="success" @click="addStu();resetForm(stuFormRef)">+ 新增学生</el-button>
    <el-button type="danger" @click="delByIds()">- 批量删除</el-button>
    <el-button type="primary" @click="sortByStudentNumber()">按学号排序</el-button>
    <el-button type="primary" @click="setDefaultSort()">默认排序</el-button>
    <br><br>
    ...
    <el-table :data="tableData" border style="width: 100%" fit @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="degree" label="最高学历" align="center" width="90px">
        <template #default="scope">
          <span v-if="scope.row.degree == 1">大专</span>
          <span v-else-if="scope.row.degree == 2">本科</span>
          <span v-else>其他</span>
        </template>
      </el-table-column>
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="姓名" :label-width="labelWidth" prop="name">
            <el-input v-model="stu.name" placeholder="请输入学生姓名"/>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="学号" :label-width="labelWidth" prop="no">
            <el-input v-model="stu.no" placeholder="请输入学生学号"/>
          </el-form-item>
        </el-col>
      </el-row>
